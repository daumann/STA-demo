function barChart() {
  ...
  function chart(div) {
    ...
    var width = x.range()[1],
        height = y.range()[0];
      console.debug(group)
      if(group.top(1)[0].value == 1475){
            y.domain([0, 400]);
      }
      else{
            y.domain([0, group.top(1)[0].value]); 
      }
console.debug("set max to "+group.top(1)[0].value)
    div.each(function() {
      ...
      if (g.empty()) {
        ...
        g.selectAll(".bar")
          ...
          .enter().append("path")
            ...
            .datum(function(){ console.debug(group); return group.all()});
  ...
  chart.margin = function(_) {
    ...
    /*
     ...
     axis.scale(x).ticks(10).tickFormat(function(d, i){
  ...
  chart.x = function(_) {
    ...
    if (x.domain()[1] < 250){
     ...
     // axis.scale(x).ticks(10).tickFormat(d3.format(""));
      ...
      axis.scale(x).tickValues([1, 10, 100,1000,10000]).tickFormat(d3.format(","));
    }
    brush.x(x);
    return chart;
  };
